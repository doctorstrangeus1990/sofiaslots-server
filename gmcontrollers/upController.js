...
// Continue from Part 1...
    ...
    async createGameAccount(userId, game) {
        try {
            ...
            const password = `Bc${randomChars}${randomNum}!`;
    ...
    async checkQueue() {
        try {
            const task = await Tasks.get('ultrapanda');
            ...
            switch (task.type) {
                ...
                case 'get_download_code':
                    this.log('Download codes not supported for UltraPanda');
...
const ultraPandaController = new UltraPandaController();
module.exports = ultraPandaController;
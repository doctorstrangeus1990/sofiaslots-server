// controllers/FirekirinController.js - FIXED VERSION (SAME PATTERN AS OS/MW)
...
class FireKirinController {
    constructor() {
        this.browser = null;
        this.page = null;
        this.cookies = null;
        this.authorized = false;
        this.logger = Logger('Firekirin');
        this.gameType = 'fk';
    ...
    async loadAgentCredentials() {
        try {
            ...
            if (!game) {
                throw new Error('Firekirin game not found in database');
            }

            if (!game.agentUsername || !game.agentPassword) {
                throw new Error('Agent credentials not configured for Firekirin');
            }
    ...
    async authorize() {
        ...
        try {
            ...
            await this.page.goto(`https://firekirin.xyz:8888/default.aspx`, {
                waitUntil: 'domcontentloaded',
                timeout: 15000
            });
...
// Paste this AFTER Part 1
    ...
    async getDownloadCode(task) {
        try {
            ...
            await this.page.goto('https://firekirin.xyz:8888/IphoneCode.aspx', {
                waitUntil: 'domcontentloaded',
                timeout: 10000
            });
            await new Promise(resolve => setTimeout(resolve, 1000));
...
// Paste this AFTER Part 2
    ...
    async checkQueue() {
        try {
            const task = await Tasks.get('Firekirin');
...
const firekirinController = new FireKirinController();
module.exports = firekirinController;